...
Nous allons donc commencer par quelques définitions paut-être évidentes pour certains, mais qui permettront de mieux comprendre la suite :
...
La tension n'est pas une grandeur qu'il est facile de définir sans entrer dans la théorie. Mais par analogie, on arrive à approcher le sens de cette grandeur. On peut faire l'analogie entre la tension électrique à un point donnée du circuit et l'énergie potentielle en mécanique.
...
Dans ce circuit, que l'on appelle un "pont diviseur de tension" (on y reviendra plus tard), si on imagine que U est la tension d'alimentation du Circuit, 5V par exemple, avant la première résistance la tension sera égale à U, soit 5V. Après la première résistance, la tension aura chutée. Après la deuxième résistance, la tension sera égale à 0.
...
La loi des mailles nous dit que lorsque l'on traverse une maille (c'est à dire une portion de circuit fermé) et partant d'un point N pour revenir au même point N, la somme des différence de potentiel rencontrées est égale à **zéro**.
...
Soit 1.66...V par résistance. Mais là vous vous demandez sûrement : "Mais 5 + 1.66 + 1.66 +1.66 ça fait 10 pas 0" et oui, vu comme ça, le résultat est bien de 10V. Mais il y a un soucis, une résistance **s'oppose** au passage du courant, donc ici la tension **chute** de 1.66V entre l'entrée et la sortie de la résistance. Si vous regardez une nouvelle fois [le schéma du pont diviseur](#la-tension-électrique), vous remarquerez que les flèches représentant les tensions U1 et U2 sont dans le sens opposé à la flèche représentant la tension d'alimentation U. C'est justement en raison du fait que la résistance s'oppose au passage du courant. Si nous reprenons donc notre calcul où nous en étions mais cette fois si en inversant les tensions aux bornes des résistances, nous obtenons :

$$5 - 1.66 - 1.66 - 1.66 = 0V$$

Ce qui correspond bien à ce que nous attendions.

**Note 1** : Pour calculer une différence de potentiel il faut simplement faire la soustraction entre la fin et le début (ce qui permet d'avoir le delta). Sur le dernier schéma, v1 peut aussi être nommée "Uab" soit "La différence de tension entre le point **a** et le point **b**" Nous pouvons donc la calculer en faisant **Uab = Ub - Ua**

**Note 2** : La loi des mailles peut être vérifiée dans les deux sens de rotation. Choisissez celui qui vont convient le mieux. Pensez simplement à mettre toutes les tensions dans le bon sens.
...
La résistance électrique est la propriété d'un conducteur à s'opposer au passage du courant. Dit plus simplement, plus la résistance d'un conducteur sera grande, et plus l'intensité du courant sera faible pour une tension donnée. A l'inverse, si la résistance du conducteur est plus faible, l'intensité sera plus forte.
...
- On peut considérer la valeur de la résistance comme étant un coefficient de proportionnalité entre la valeur de la tensio net l'intensité. Si l'on trace la caractéristique d'une résistance (c'est à dire la courbe de la tension en fonction de l'intensité) nous obtiendrons une droite passaant par l'origine et donc la pente sera la valeur de la résistance.
